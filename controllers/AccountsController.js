...
export default class AccountsController extends Controller {
    ...
    // POST: /token body payload[{"Email": "...", "Password": "..."}] (les informations de l'usager)
    login(loginInfo) {
        if (loginInfo) {
            if (this.repository != null) {
                let user = this.repository.findByField("Email", loginInfo.Email);
                if (user != null) {
                    if (user.Password == loginInfo.Password) { //On vérifie la correspondance avec le password
                        user = this.repository.get(user.Id);
                        let newToken = TokenManager.create(user); //Retourne le bearer token et les informations de l'usager
